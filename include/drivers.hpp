...
namespace driver {
    ...
    void gaxpy(size_t n, T alpha, std::vector<T> const &x, std::vector<T> &y) {
        std::cout << "Classic GAXPY with " << n << " elements on CPU" << std::endl;
        std::vector<f64> samples(META_REPS);
        high_resolution_clock::time_point start, end;

        // Warmup loop
        for (size_t _ = 0; _ < WARMUP_REPS; ++_) {
            kernel::gaxpy(n, alpha, x, y);
        }
        ...
        auto mean = utils::stats::mean(samples);
        auto stddev = utils::stats::stddev(samples);
        std::cout << "Average: " << mean.count() << " +/- " << stddev.count() << " µs" << std::endl;
    }

    template <typename T = f32>
    void sycl_gaxpy(size_t n, T alpha, std::vector<T> const &x, std::vector<T> &y) {
        ...
        std::cout << "SYCL GAXPY with " << n << " elements on "
        ...
        try {
            for (size_t w = 0; w < WARMUP_REPS; ++w) {
                q.parallel_for(sycl::range<1>(MAX_THREADS),
                               [=](sycl::id<1> i) { sy[i] += alpha * sx[i]; });
            }
            for (size_t m = 0; m < META_REPS; ++m) {
                start = high_resolution_clock::now();
                for (size_t j = 0; j < REPS; ++j) {
                    q.parallel_for(sycl::nd_range<1>(MAX_THREADS, 64),
                                   [=](sycl::id<1> i) { sy[i] += alpha * sx[i]; });
                }
                end = high_resolution_clock::now();
